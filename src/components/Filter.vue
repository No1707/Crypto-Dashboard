<template>
    <div class="px-4 py-2 md:p-4 flex flex-wrap sm:flex-nowrap items-center w-full sticky top-[45px] md:top-[70px] bg-white dark:bg-slate-800 transition-shadow z-10">
        ...
        <!-- statut refresh -->
        <div class="ml-auto flex items-center gap-1.5 font-bold text-base text-black dark:text-white whitespace-nowrap">
            <span class="w-1.5 h-1.5 rounded-full" :class="isLoading ? 'bg-amber-400 animate-pulse' : 'bg-green-400'"></span>
            <span v-if="isLoading">Mise à jour…</span>
            <span v-else-if="statusText">Mis à jour il y a {{ statusText }}</span>
        </div>
